...
router.get('/', requireAuth, (req, res) => {
  ...
  const result = servers.map(s => {
    const latest = db.prepare(
      'SELECT * FROM stats WHERE server_id = ? ORDER BY ts DESC LIMIT 1'
    ).get(s.id)
    return {
      id:             s.id,
      name:           s.name,
...
router.get('/:id', requireAuth, (req, res) => {
  ...
  const recentEvents = db.prepare(
    'SELECT * FROM player_events WHERE server_id = ? ORDER BY ts DESC LIMIT 50'
  ).all(server.id)
  const recentDeaths = db.prepare(
    'SELECT * FROM death_events WHERE server_id = ? ORDER BY ts DESC LIMIT 50'
  ).all(server.id)
...
router.get('/:id/graph', requireAuth, (req, res) => {
  ...
  const metric = ['tps','player_count','memory_used','mspt'].includes(req.query.metric)
    ? req.query.metric : 'tps'
