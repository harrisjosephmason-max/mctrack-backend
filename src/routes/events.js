...
router.post('/stats', requireApiKey, (req, res) => {
  ...
  db.prepare(`INSERT INTO stats (server_id,tps,player_count,max_players,uptime_seconds,memory_used,memory_max,mspt,online) VALUES (?,?,?,?,?,?,?,?,1)`)
...
router.post('/kill', requireApiKey, (req, res) => {
  ...
  db.prepare('INSERT INTO kill_events (server_id,killer_name,killer_uuid,victim_name,victim_uuid) VALUES (?,?,?,?,?)')
    .run(req.server.id, killer_name, killer_uuid||'', victim_name, victim_uuid||'')
  res.json({ ok: true })
})
...
router.post('/command', requireApiKey, (req, res) => {
  ...
  db.prepare('INSERT INTO command_events (server_id,player_name,player_uuid,command) VALUES (?,?,?,?)')
    .run(req.server.id, player_name, player_uuid||'', command)
  res.json({ ok: true })
})
