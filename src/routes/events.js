...
router.post('/player', requireApiKey, (req, res) => {
  ...
  db.prepare(
    'INSERT INTO player_events (server_id, player_name, player_uuid, event_type) VALUES (?, ?, ?, ?)'
...
router.post('/chat', requireApiKey, (req, res) => {
  ...
  db.prepare(
    'INSERT INTO chat_messages (server_id, player_name, player_uuid, message) VALUES (?, ?, ?, ?)'
...
router.get('/chat/:serverId', require('../middleware/auth'), (req, res) => {
  ...
  const rows = db.prepare(
    'SELECT * FROM chat_messages WHERE server_id = ? ORDER BY ts DESC LIMIT 100'
  ).all(server.id)
  res.json(rows)
})
...
router.post('/death', requireApiKey, (req, res) => {
  ...
  db.prepare(
    'INSERT INTO death_events (server_id, player_name, player_uuid, death_message) VALUES (?, ?, ?, ?)'
...
router.post('/offline', requireApiKey, (req, res) => {
  db.prepare(
    'INSERT INTO stats (server_id, tps, player_count, online) VALUES (?, 0, 0, 0)'
  ).run(req.server.id)
  res.json({ ok: true })
})
