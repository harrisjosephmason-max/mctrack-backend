...
const { onUserJoin } = require('../discord')
  ...
  try {
    ...
    if (!user) {
      ...
      db.prepare('INSERT INTO users (id, discord_id, username, avatar) VALUES (?, ?, ?, ?)')
        .run(id, du.id, du.username, du.avatar||'')
      user = db.prepare('SELECT * FROM users WHERE id = ?').get(id)
    } else {
      db.prepare('UPDATE users SET username = ?, avatar = ? WHERE discord_id = ?')
        .run(du.username, du.avatar||'', du.id)
    }

    // Discord notification
    const avatarUrl = du.avatar ? `https://cdn.discordapp.com/avatars/${du.id}/${du.avatar}.png` : null
    onUserJoin(du.username, avatarUrl).catch(() => {})

    const token = Buffer.from(`${user.discord_id}:${user.id}`).toString('base64')
    res.redirect(`${FRONTEND_URL}/auth/callback?token=${token}&username=${encodeURIComponent(user.username)}&avatar=${user.avatar||''}&discord_id=${user.discord_id}`)
  } catch (err) {
